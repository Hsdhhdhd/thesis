...
Incoming notifications are parsed into converted acceleration and angular-velocity channels using the same receiver-side unit conversion as the Android backend. Additional derived channels, including gravity-removed motion acceleration and quaternion estimates, are computed in the receiver pipeline for later analysis.
...
The PC receiver writes its own CSV file with raw and derived signals (Table~\ref{tab:pc_csv_format}). Compared with the Android log it omits annotation and GPS fields but adds gravity-removed motion acceleration and quaternion estimates that were useful for offline bench checks.
...
Before the pilot tests, the PC receiver backend was used to confirm BLE connection, six-axis packet decoding, packet-to-file logging, and recording stability. These checks made it possible to test the wearable on the bench before using the Android backend for mobile field collection.
