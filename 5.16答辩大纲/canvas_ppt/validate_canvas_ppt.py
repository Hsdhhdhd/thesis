#!/usr/bin/env python3
"""Validate the 5.16 Canvas PPT without needing a browser."""
...
REPO_ROOT = CANVAS_DIR.parents[1]
INDEX = CANVAS_DIR / "index.html"
EXPECTED_SLIDES = 28
ASSET_BASES = {
    "A": REPO_ROOT / "5.12论文重写" / "images",
...
def check_assets(script: str) -> None:
    ...
    if not refs:
        raise AssertionError("No image asset references found")
    missing: list[Path] = []
...
def main() -> int:
    ...
    check_assets(script)
    print(f"OK: {INDEX.relative_to(REPO_ROOT)} has {EXPECTED_SLIDES} slides and all referenced assets exist")
    return 0
